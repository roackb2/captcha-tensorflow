...
LOG_DIR = 'log/cnn4-run-%s' % datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
...
def main(_):
    ...
    with tf.name_scope('input'):
        ...
        y_ = tf.placeholder(tf.float32, [None, NUM_PER_IMAGE * LABEL_SIZE], name='label-input')

        # must be 4-D with shape `[batch_size, height, width, channels]`
        x_image = tf.reshape(x, [-1, IMAGE_HEIGHT, IMAGE_WIDTH, 1], name='x-input')
    ...
    with tf.name_scope('reshape'):
        ...
        y_got_reshaped = tf.reshape(y_conv, [-1, NUM_PER_IMAGE, LABEL_SIZE], name='predict')
        predict_max_idx = tf.argmax(y_got_reshaped, axis=2, name='predict_max_idx')
    ...
    with tf.name_scope('forword-prop'):
        ...
        expect = tf.argmax(y_expect_reshaped, axis=2, name='expect')
    ...
    with tf.Session() as sess:
        ...
        for i in range(MAX_STEPS):
            ...
            if i % 100 == 0:
                ...
                if test_accuracy > 0.4:
                    saver.save(sess, "model/crack_captcha.model", global_step=i)
